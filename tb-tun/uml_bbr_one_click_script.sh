...
function fun_start() {
    ...
    setsid ${APP_DIR}/tb_userspace ${IFNAME} ${Server_IPv4_Address} ${Client_IPv4_Address} sit > /dev/null 2>&1 &
    if [ $? -ne 0 ]; then echo "[ERROR]:${LINENO}"; return 1; fi
...
function fun_install() {
    ...
    wget --directory-prefix=${APP_DIR} https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/tb-tun/tb-tun_r18.tar.gz  > /dev/null 2>&1
    if [ $? -ne 0 ]; then echo "[ERROR]:${LINENO}"; return 1; fi
